...
-- fix wrong Dept.
update sct_64x_n
set Department64 = replace(Department64,'GENERAK','GENERAL')
where Department64 like 'GENERAK';

select distinct Department64
from sct_64x_n sxn
order by Department64;
...
SELECT month(PostingDate64) as `Month`,Account64, Line64, 
PostingDate64, TransID64, TaxDate64, Description64, 
OffstAcct64, Debit64, Credit64, NetAmt64, Branch64, CostCenter64, 
Department64, Mft64
FROM sct_64x_n
where Department64 in ('KDD') -- 'GERMANOS','MESOLINE','KDHC','ALPECIN','HD'
-- where Department64 like 'GSDP'
-- and CostCenter64 in ('ITNET','ICLOD')
and month(PostingDate64)=9
-- and Branch64 in ('CN') -- 'CS','GS','S1','SD','CC',''
order by `Month`;
...
Department64, Mft64
FROM sct_64x_n
-- where NetAmt64 >= 3250000
where CostCenter64 in ('CBTSC')
and Department64 like 'GSDP'
 and month(PostingDate64)=8
order by `Month`;
